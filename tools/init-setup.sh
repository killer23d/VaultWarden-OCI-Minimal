...
# CONFIG_FILE is now sourced from lib/config.sh
...
_init_setup_workflow() {
    ...
    # --- FIX: Reorder function calls to fix race condition ---
    # Configure Fail2ban AFTER settings.json is created.
...
_setup_cron_jobs() {
    ...
    cat > "$cron_file" <<EOF
# VaultWarden OCI Minimal - Automated Maintenance Schedule
