...
_fetch_cloudflare_ips() {
  ...
  while (( i < max_retries )); do
    ...
    if (( i < max_retries )); then
      _log_warning "Retrying in ${retry_delay}s..."
      sleep "$retry_delay"
    fi
  done
...
_generate_caddy_config() {
  ...
  cat <<CADDY
# Generated by update-cloudflare-ips.sh
# IPv4 ranges
$(echo "$ipv4_ranges" | awk 'NF' | sort -u | sed 's/^/remote_ip /')
# IPv6 ranges
$(echo "$ipv6_ranges" | awk 'NF' | sort -u | sed 's/^/remote_ip /')
CADDY
}
...
_update_cloudflare_ips() {
  ...
  ipv4_ranges="$(_fetch_cloudflare_ips "$CF_IPV4_URL" "IPv4")" || { _log_error "Failed to fetch IPv4 ranges"; return 1; }
  ipv6_ranges="$(_fetch_cloudflare_ips "$CF_IPV6_URL" "IPv6")" || { _log_error "Failed to fetch IPv6 ranges"; return 1; }
